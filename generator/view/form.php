    ...
    @section('content')
        ...
        <select id="garden_id" name="garden_id" class="form-control form-control-sm">
            <option selected="selected" value="">{{ trans('lang.garden_id') }} নির্বাচন করুন...</option>

            @foreach (#forms as #form)
                <option {{#form->id == old('garden_id') ? 'selected' : ''}} value="{{ #form->id }}">{{ #form->name }}
                </option>
            @endforeach
        ...
        <select id="voter_id" name="voter_id" class="form-control form-control-sm">
            <option selected="selected" value="">{{ trans('lang.voter_id') }} নির্বাচন করুন...</option>

            @foreach (#forms as #form)
                <option {{#form->id == old('voter_id') ? 'selected' : ''}} value="{{ #form->id }}">{{ #form->name }}
                </option>
            @endforeach
        ...
        <input class="form-control" name="mobile" value="{{old('mobile')}}" type="text" id="mobile">
    </div>    <div class="form-group">
        <label for="details" class="form-control-label">{{trans('lang.details')}}</label>
        <input class="form-control" name="details" value="{{old('details')}}" type="text" id="details">
    </div>                                </div>
                            </div>
                        </div>
                        <div class="pl-lg-4">
